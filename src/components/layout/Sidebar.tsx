...
export default function Sidebar({ profile }: { profile: Profile }) {
  ...
  const nav = profile.role === 'super_admin' || profile.role === 'admin'
    ? SUPER_ADMIN_NAV
    : profile.role === 'profe'
      ? PROFE_NAV
      : ALUMNO_NAV
  ...
  return (
    <aside style={{
      width: 'var(--sidebar-w)', background: '#0e0e0e',
      borderRight: '1px solid rgba(255,255,255,0.06)', position: 'fixed',
        ...
        {nav.map((item: any, i) => {
          ...
          return (
            <a key={item.href} href={item.href} style={{
              ...
              color: isActive ? '#c8f542' : '#666', borderLeft: `2px solid ${isActive ? '#c8f542' : 'transparent'}`,
              background: isActive ? 'rgba(200,245,66,0.05)' : 'transparent', textDecoration: 'none', transition: 'all 0.15s',
            }}>
      ...
      <div style={{ padding: 16, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        ...
        <button onClick={handleLogout} disabled={loggingOut}
          style={{ width: '100%', padding: '8px', background: 'transparent', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, color: '#555', fontSize: 12, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>
