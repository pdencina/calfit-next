...
export default function ProfeMensajesPage() {
  ...
  async function load() {
    ...
    const { data: conv } = await supabase.from('conversations').select('*').eq('org_id', resolvedOrgId).eq('profe_id', uid).order('last_message_at', { ascending: false })
    const list = (conv || []) as Conversation[]
  ...
  async function createConversation() {
    if (!orgId || !userId || !selectedStudent) return
    setError('')
    const { data, error: upsertError } = await supabase
      .from('conversations')
      .upsert({ org_id: orgId, profe_id: userId, alumno_id: selectedStudent }, { onConflict: 'org_id,profe_id,alumno_id' })
      .select('*')
      .single()
    if (upsertError) setError(upsertError.message)
    else { await load(); setSelectedConversation(data.id) }
  }
  ...
  async function sendMessage(e: React.FormEvent) {
    ...
    setContent('')
    const { error: sendError } = await supabase.from('messages').insert({ conversation_id: selectedConversation, sender_id: userId, content: text })
    if (sendError) { setError(sendError.message); setContent(text) }
    else await loadMessages(selectedConversation)
  }

  const selected = conversations.find(c => c.id === selectedConversation)
  const selectedStudentProfile = selected ? students.find(s => s.id === selected.alumno_id) : null
  ...
  return (
    <div>
      ...
      <div className="page-sub">Comunicación directa entre profesor y alumno.</div>
      {error && <div className="alert-error" style={{ marginBottom: 16 }}>{error}</div>}

      <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: 18 }}>
        <div className="card">
          <div style={{ fontSize: 12, letterSpacing: 1.5, textTransform: 'uppercase', color: '#666', marginBottom: 14 }}>Conversaciones</div>
          <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
            <select value={selectedStudent} onChange={e => setSelectedStudent(e.target.value)} style={field}>
              {students.map(s => <option key={s.id} value={s.id}>{s.full_name || s.email}</option>)}
            </select>
            <button className="btn btn-primary" onClick={createConversation}>+</button>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {conversations.map(c => {
              const st = students.find(s => s.id === c.alumno_id)
              return <button key={c.id} onClick={() => setSelectedConversation(c.id)} className="btn btn-ghost" style={{ justifyContent: 'flex-start', borderColor: c.id === selectedConversation ? '#c8f542' : 'rgba(255,255,255,0.1)', color: c.id === selectedConversation ? '#c8f542' : '#888' }}>
                <span style={{ width: 28, height: 28, borderRadius: '50%', background: '#222', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>{(st?.full_name || 'A')[0]}</span>
                {st?.full_name || st?.email || 'Alumno'}
              </button>
            })}
            {!conversations.length && <div style={{ color: '#555', fontSize: 13 }}>Crea la primera conversación.</div>}
          </div>
        </div>
        ...
        <div className="card" style={{ minHeight: 560, display: 'flex', flexDirection: 'column' }}>
          {selected ? <>
            <div style={{ paddingBottom: 14, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
              <div style={{ fontFamily: 'Bebas Neue', letterSpacing: 2, fontSize: 26 }}>{selectedStudentProfile?.full_name || 'Alumno'}</div>
              <div style={{ color: '#666', fontSize: 12 }}>{selectedStudentProfile?.email}</div>
            </div>
            <div style={{ flex: 1, overflowY: 'auto', padding: '18px 0', display: 'flex', flexDirection: 'column', gap: 10 }}>
              {messages.map(m => {
                const own = m.sender_id === userId
                return <div key={m.id} style={{ alignSelf: own ? 'flex-end' : 'flex-start', maxWidth: '72%' }}>
                  <div style={{ background: own ? '#c8f542' : '#222', color: own ? '#070707' : '#f0efe8', padding: '10px 14px', borderRadius: 14, fontSize: 14, lineHeight: 1.4 }}>{m.content}</div>
          ...
          </> : <div style={{ color: '#555', textAlign: 'center', padding: 80 }}>Selecciona o crea una conversación.</div>}
        </div>
      </div>
    </div>
  )
