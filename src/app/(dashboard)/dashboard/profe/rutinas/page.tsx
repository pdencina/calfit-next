...
export default function ProfeRutinasPage() {
  ...
  function alreadyUsed(templateId: string) {
    return myRoutines.some(
      (r) =>
        r.template_source_id === templateId
    )
  }
  ...
  async function createFromTemplate(templateId: string) {
    ...
    const templateExercises =
      getExercises(template.id)

    if (templateExercises.length > 0) {
      const payload = templateExercises.map(
        (e) => ({
      ...
      <div style={styles.templatesGrid}>
        {filteredTemplates.map((routine) => (
          ...
          >
            ...
            <p style={styles.meta}>
              ⏱️ {routine.duracion_min} min ·{' '}
              {getExercises(routine.id).length}{' '}
              ejercicios
            </p>
